...
def collect_data(content_ids: Optional[List[int]] = Query(default=None), session: Session = Depends(get_sync_session)):
...
def get_overview(session: Session = Depends(get_sync_session)):
...
def get_suggestions(session: Session = Depends(get_sync_session)):
