...
def batch_distribute(
    ...
    return [DistributionResponse.model_validate(d) for d in items]
...
def publish_distribution(distribution_id: int, session: Session = Depends(get_sync_session)):
    """模拟发布待发布的分布记录"""
    return DistributionResponse.model_validate(distribution_service.publish_distribution(session, distribution_id))
...
def cancel_distribution(distribution_id: int, session: Session = Depends(get_sync_session)):
    """取消待发布或已排期的分发"""
    return DistributionResponse.model_validate(distribution_service.cancel_distribution(session, distribution_id))
